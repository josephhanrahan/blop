...
![CHX_SRW_layout.png](../_static/CHX_SRW_layout.png)
...
dofs = [
    RangeDOF(actuator=beamline.crl2_xoff, bounds=VERTICAL_BOUNDS, parameter_type="float"),
    RangeDOF(actuator=beamline.crl2_yoff, bounds=HORIZONTAL_BOUNDS, parameter_type="float"),
]
```
...
<!-- - **Spot width X** (`bl_det_crl2_xoff`): We want a *tighter* spot → `minimize=True`
- **Spot width Y** (`bl_det_crl2_yoff`): We want a *tighter* spot → `minimize=True` -->
